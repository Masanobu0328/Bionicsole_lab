...
async def debug_env_check():
    """Report which auth variables the running process can see - names only.

    Temporary. Railway reported the two Supabase variables as set, yet the
    process still saw neither, so guessing at the cause was costing round
    trips. This says which service and which commit is actually serving, and
    whether the names match, without ever putting a value on the wire.
    """
    import os as _os
    ...
    def describe(name: str) -> Dict[str, object]:
        ...
        return {
            ...
            "has_surrounding_whitespace": bool(raw) and raw != raw.strip(),
        }
    ...
    return {
        ...
        # Any near-miss name (a typo, a NEXT_PUBLIC_ prefix) shows up here.
        "supabase_like_names": sorted(
            k for k in _os.environ if "SUPA" in k.upper()
        ),
        "railway": {
            k: _os.environ.get(k)
            for k in (
