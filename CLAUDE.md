...
**Y軸の「0/1」は正規化された比率（`arch_y_ratio`）であって、生のy座標ではない。**
生の座標では逆で、`geometry_v4_frontend.py` の中では **y最小側が内側（medial）**。

```python
y_ratio      = (y - y_min_at_x) / local_width   # y最小側が0
arch_y_ratio = 1.0 - y_ratio                    # y最小側が1 = 内側
```

この2つを混同すると「アーチが内外逆」に見える。実際に一度そう誤診している。

---

## 左右の足（これを間違えると全部の印刷物が無駄になる）

**編集する輪郭は「左足」の基準形**。右足のメッシュだけを鏡像にする。

`backend/api/endpoints.py` の `MIRRORED_FOOT_SIDE = "right"` がそれ。
`tests/test_foot_side.py` が固定しているので、変えるとテストが落ちる。

**Why:** 2026-09-13、印刷したら左右が入れ替わっていた。右のファイルが左のインソールだった。

**How to apply:**

- エンジンは**メッシュを1つだけ作り**、この鏡像処理が**アーチごと全体を反転**する。
  測定値で、片方を戻すと両者は **0.0000mm** 一致する。
  つまり各出力はそれ自体では破綻していない。**狂うとしたらラベルの対応だけ**
- したがって「アーチが内外逆」に見えたときも、**まず左右のラベルを疑う**。
  アーチ側の判定（上記 `arch_y_ratio`）は初回コミットから変わっていない
- 手描き輪郭は `patients/0001/outline.csv`（実スキャン）と**同じ向き**であることを確認済み。
  なので分岐は1箇所でよく、患者ごとの場合分けは不要
- **形状データだけから「物理的にどちらの足か」は判定できない**。3通り試して全部失敗した。
  判定は実物を印刷して確かめるのが早い
